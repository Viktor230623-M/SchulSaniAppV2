...
interface ApiErrorShape { response?: { data?: { error?: string } } }
import {
  View,
  FlatList,
  TouchableOpacity,
...
export default function MissionsScreen() {
  ...
  const { data: missions, isLoading, error, refetch } = useGetMissions();
  ...
  const renderItem = ({ item }: { item: Mission }) => {
    ...
    return (
      ...
      >
        <Card style={isActive ? [styles.activeCard, { borderColor: theme.danger }] : undefined}>
  ...
  return (
    <View style={[styles.container, { backgroundColor: theme.background }]}>
      ...
      {!isLoading && user?.role === "sanitaeter" && (error as ApiErrorShape)?.response?.data?.error === "no_duty_today" ? (
        <EmptyState
          icon="alert-circle"
          title={t.missions.noDutyAccess}
          subtitle={t.missions.noDutyAccessDesc}
        />
      ) : isLoading ? (
