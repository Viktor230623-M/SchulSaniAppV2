...
export const dutyEntriesTable = pgTable("duty_entries", {
  id: uuid("id").primaryKey().defaultRandom(),
  userId: uuid("user_id")
    .notNull()
    .references(() => usersTable.id, { onDelete: "cascade" }),
  dutySlotId: uuid("duty_slot_id")
    .notNull()
    .references(() => dutySlotsTable.id, { onDelete: "cascade" }),
  date: date("date").notNull(),
...
export const usersRelations = relations(usersTable, ({ one, many }) => ({
  ...
  dutyEntries: many(dutyEntriesTable),
...
export const dutySlotsRelations = relations(dutySlotsTable, ({ one, many }) => ({
  user: one(usersTable, {
    fields: [dutySlotsTable.userId],
    references: [usersTable.id],
  }),
  entries: many(dutyEntriesTable),
}));

export const dutyEntriesRelations = relations(dutyEntriesTable, ({ one }) => ({
  user: one(usersTable, {
    fields: [dutyEntriesTable.userId],
    references: [usersTable.id],
  }),
  slot: one(dutySlotsTable, {
    fields: [dutyEntriesTable.dutySlotId],
    references: [dutySlotsTable.id],
  }),
}));
...
export const insertDutyEntrySchema = createInsertSchema(dutyEntriesTable).omit({
  id: true,
  enteredAt: true,
  confirmedAt: true,
});
export type InsertDutyEntry = z.infer<typeof insertDutyEntrySchema>;
export type DutyEntry = typeof dutyEntriesTable.$inferSelect;
