...
const SYSTEM_ROLES = ["admin", "wachleiter", "sanitaeter"] as const;
...
export default function AdminUsersScreen() {
  ...
  const { data: customRoles } = useGetCustomRoles({ query: { enabled: isOwner } });
  ...
  const roleLabel = (role: string) => {
    const map: Record<string, string> = {
      ...
      wachleiter: t.profile.roles_label.wachleiter,
  ...
  const roleVariant = (role: string) => {
    ...
    if (role === "wachleiter") return "success" as const;
              ...
              {(customRoles?.length ?? 0) > 0 && (
                <>
                  ...
                  {selected?.customRoleId && (
                    ...
                    >
                      <Text style={[{ color: theme.danger, fontFamily: "Inter_500Medium", fontSize: 14 }]}>
                        Benutzerdefinierte Rolle entfernen
                      </Text>
                    </TouchableOpacity>
                  )}
                </>
